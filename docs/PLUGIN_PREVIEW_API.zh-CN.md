...
仍需声明引用的 `component.fullPage`。插件面板直接加入文件夹、预览、详细信息使用的统一面板列表，不再单独分组。菜单和顶部栏共享固定/关闭状态，支持与原生面板混合拖动排序、Alt+左右键排序、相邻边界缩放。多个插件面板可同时显示；关闭或切换文件页时隐藏视图，重新打开保留正文实例，关闭来源页或卸载时释放。固定状态、排序和宽度会保存；当前源码中新插件面板默认打开，点击文件区空白处不会收起插件面板。“恢复默认布局”统一重置顺序/宽度，恢复原生面板，并以未固定状态打开插件面板。
