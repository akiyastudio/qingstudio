// Generated from src/i18n/legacy-parser.ts. Do not edit.
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.createLegacyParser = void 0;
const createLegacyParser = (catalogs, baseLanguage = 'zh-CN') => {
    const legacyKeys = new Map();
    const legacyTemplates = [];
    for (const catalog of Object.values(catalogs))
        for (const [key, text] of Object.entries(catalog)) {
            if (!text.includes('{')) {
                if (!legacyKeys.has(text))
                    legacyKeys.set(text, key);
                continue;
            }
            ...
            for (const match of matches) {
                literals.push(text.slice(offset, match.index));
                offset = match.index + match[0].length;
            }
            literals.push(text.slice(offset));
            if (matches.length && literals.join('').length >= 4 && (literals[0].trim() || literals.at(-1).trim()) && literals.slice(1, -1).every(Boolean))
                legacyTemplates.push({ key: (Object.prototype.hasOwnProperty.call(catalog, key.replace(/\.(zero|one|two|few|many|other)$/, '')) ? key.replace(/\.(zero|one|two|few|many|other)$/, '') : key), literals, names: matches.map(match => match[1]) });
        }
    ...
    const parse = (source) => {
        if (/^(?:[A-Za-z]:[\\/]|\\\\|\/|[a-z][a-z0-9+.-]*:\/\/)/i.test(source))
            return undefined;
        ...
        for (const template of legacyTemplates) {
            if (!source.startsWith(template.literals[0]) || !source.endsWith(template.literals.at(-1)))
                continue;
            ...
            for (const literal of new Set(middle)) {
                ...
                const expected = template.literals.reduce((count, part) => { let at = 0; while ((at = part.indexOf(literal, at)) >= 0) {
                    count++;
                    at += 1;
                } return count; }, 0);
            ...
            if (ambiguous)
                continue;
            let offset = template.literals[0].length;
            let valid = true;
            const params = {};
            for (let index = 0; index < template.names.length; index++) {
                ...
                if (name === 'count' && (!/^-?\d+(?:\.\d+)?$/.test(value) || !Number.isFinite(Number(value)))) {
                    valid = false;
                    break;
                }
                const parsed = name === 'count' ? Number(value) : value;
                if (Object.prototype.hasOwnProperty.call(params, name) && params[name] !== parsed) {
                    valid = false;
                    break;
                }
                params[name] = parsed;
            ...
            if (valid && offset === source.length) {
                const base = catalogs[baseLanguage][template.key] ?? '';
                ...
                for (const name of template.names) {
                    ...
                    if (/失败|错误|异常/.test(prefix) && /[：:]\s*$/.test(prefix) && typeof value === 'string') {
                        const detailKey = legacyKeys.get(value) ?? legacyKeys.get(value.replace(/^(?:Error|TypeError|RangeError):\s*/, ''));
                        if (detailKey)
                            parameterMessages[name] = detailKey;
                    }
                }
...
exports.createLegacyParser = createLegacyParser;
