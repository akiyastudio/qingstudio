    ...
    {showExistingProjectImport && <ProjectImportDialog title="导入项目" busy={isImportingExistingProject} onClose={closeExistingProjectImport}>
      {existingProjectResult && existingProjectDraft ? <div className="space-y-4">
        <section className={`rounded-xl border px-4 py-4 ${existingProjectResult.sourceRetained ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
          <div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${existingProjectResult.sourceRetained ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}><FolderInput size={18}/></span><div><h4 className="text-sm font-bold text-slate-800">项目导入完成</h4><p className="mt-1 text-xs leading-5 text-slate-600">项目已接入“策划中”{existingProjectResult.sourceRetained ? '；源项目有内容未能安全清理，仍保留在原位置。' : '，文件夹按普通文件管理。'}</p></div></div>
          <div className="mt-4 grid grid-cols-2 gap-2">{[['文件', existingProjectDraft.fileCount.toLocaleString()], ['文件夹', existingProjectDraft.folderCount.toLocaleString()]].map(([label, value]) => <div key={label} className="rounded-lg border border-white/80 bg-white/75 px-3 py-2"><span className="block text-[10px] text-slate-400">{label}</span><b className="mt-1 block text-sm text-slate-700">{value}</b></div>)}</div>
        </section>
      ...
      </div> : !existingProjectDraft ? <div className="space-y-4"><div onDragOver={event => { if (!event.dataTransfer.types.includes('Files')) return; event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; setExistingProjectDragActive(true); }} onDragLeave={() => setExistingProjectDragActive(false)} onDrop={event => void inspectDroppedExistingProject(event)} className={`grid min-h-64 place-items-center rounded-xl border border-dashed p-8 text-center transition ${existingProjectDragActive ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-500/20' : 'border-slate-300 bg-slate-50'}`}><div><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-blue-100 text-blue-600"><FolderInput size={22}/></span><p className="mt-4 text-sm font-bold text-slate-800">拖入一个项目或素材文件夹</p><p className="mt-1 text-xs text-slate-500">或使用下面的按钮从磁盘选择</p><button type="button" disabled={choosingExistingProject} onClick={() => void chooseExistingProject()} className="dialog-primary mt-4 inline-flex items-center gap-2">{choosingExistingProject && <Loader2 size={15} className="animate-spin"/>}选择文件夹</button></div></div>{existingProjectError && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-600">{existingProjectError}</div>}</div> : <form className="space-y-4" onSubmit={event => { event.preventDefault(); void importExistingProject(existingProjectMode); }}>
        ...
        <fieldset disabled={isImportingExistingProject}>
          <legend className="mb-2 text-xs font-bold text-slate-600">导入方式</legend>
          <div className="grid gap-2 sm:grid-cols-3">
            {([
              ['copy', '复制', '复制到工作目录，保留原文件。'],
              ['move', '移动', '复制并校验后，移除原位置的文件。'],
              ['reference', '引用原位置', '文件留在原处；修改和删除会作用于原文件。'],
        ...
        <p className="text-xs leading-5 text-slate-500">保留文件夹结构，内容按普通文件管理。</p>
        {isImportingExistingProject && <section className="rounded-xl border border-slate-200 bg-slate-50 p-3" aria-live="polite">
          <p className="text-xs text-slate-600">{existingProjectImportTask?.message || (existingProjectMode === 'reference' ? '正在添加项目…' : '正在导入项目…')}</p>
