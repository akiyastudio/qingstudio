...
export const WindowControls = () => {
  ...
  return <div className="app-titlebar-control flex h-10 w-[138px] shrink-0 items-stretch">
    <button type="button" onClick={() => window.electronAPI.minimizeWindow()} aria-label={t("ui.minimize.ac29e5")} title={t("ui.minimize.ac29e5")} className="window-control-button"><span className="window-glyph window-glyph-minimize"/></button>
    <button type="button" onClick={async () => setMaximized(await window.electronAPI.toggleMaximizeWindow())} aria-label={maximized ? t("ui.restore.ddde08") : t("ui.maximize.82c4aa")} title={maximized ? t("ui.restore.ddde08") : t("ui.maximize.82c4aa")} className="window-control-button">{maximized ? <span className="window-glyph window-glyph-restore"/> : <span className="window-glyph window-glyph-maximize"/>}</button>
    <button type="button" onClick={() => window.electronAPI.closeWindow()} aria-label={t("common.close")} title={t("common.close")} className="window-control-button window-control-close"><span className="window-glyph window-glyph-close"/></button>
  </div>;
};

export const StartupWindowFrame = ({ children }: { children: ReactNode }) => { useLocale(); return (<div className="h-screen w-full overflow-auto">
  <header className="startup-window-titlebar fixed inset-x-0 top-0 z-[2000] flex h-10 items-stretch"><div title={t("ui.drag.window.5cfffe")} className="app-window-drag-region min-w-0 flex-1"/><WindowControls/></header>
  {children}
</div>); };
...
export const BackupHomeCard = ({ status, onOpen, onRun }: { status: BackupStatus; onOpen: () => void; onRun: () => void }) => {
  ...
  const detail = running ? status.task?.message || '正在准备备份'
    : protectedState && status.latestAt ? `上次成功：${new Date(status.latestAt).toLocaleString(getLocale())}`
      : offline ? status.targetPath || '连接备份盘后会自动恢复状态'
        : status.error || '保护项目文件、版本关系和软件设置';
  const displayedDetail = offline && status.targetPath ? detail : renderText(detail);
  ...
  return <div className="group flex min-w-0 items-center gap-4 rounded-xl border border-slate-200 bg-white px-5 py-5 transition hover:border-blue-400 hover:bg-blue-50 hover:shadow-sm">
    <button type="button" onClick={onOpen} className="flex min-w-0 flex-1 items-center gap-4 text-left">
      ...
      <span className="min-w-0 flex-1"><span className="block text-base font-bold text-slate-800">{renderText(title)}</span><span title={displayedDetail} className="mt-1 block truncate text-xs text-slate-500">{displayedDetail}</span></span>
    </button>
    {status.enabled && !running && <button type="button" onClick={onRun} className="shrink-0 rounded-lg border border-blue-200 bg-white px-3 py-2 text-xs font-bold text-blue-600 hover:bg-blue-50">{t("ui.back.up.now.e5c63c")}</button>}
...
export const UpdateModal = ({ version, notes, url, mandatory, onClose }: AppUpdateInfo & { onClose: () => void }) => {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
    <div ref={dialogRef} tabIndex={-1} onKeyDown={trapFocus} role="dialog" aria-modal="true" aria-label={`${mandatory ? t("ui.required.update.db4514") : t("ui.new.version.available.ac217e")} ${version}`} className="bg-white border border-blue-500/30 w-full max-w-md rounded-2xl shadow-2xl flex flex-col relative overflow-hidden">
      ...
      <div className="p-6 pb-0 z-10"><div className="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center text-blue-600 mb-4 border border-blue-500/20"><Gift size={24}/></div><h3 className="text-xl font-bold text-slate-800 mb-2">{mandatory ? t("ui.update.required.4be1c0") : t("ui.new.version.available.ac217e")} {version}</h3><p className="text-slate-500 text-sm">{mandatory ? t("ui.this.update.is.required.install.it.9187c4") : t("ui.an.update.is.ready.download.the.9857a2")}</p></div>
      <div className="p-6 z-10"><div className="bg-slate-50/50 rounded-lg p-4 border border-slate-200 max-h-40 overflow-y-auto"><p className="text-xs font-bold text-slate-500 uppercase mb-2">{t("ui.release.notes.de6811")}</p><p className="text-sm text-slate-800 whitespace-pre-wrap">{notes}</p></div></div>
      <div className="p-6 pt-2 flex gap-3 z-10">{!mandatory && <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-lg text-slate-500 hover:text-slate-800 hover:bg-slate-100 transition font-medium text-sm">{t("ui.later.9d2055")}</button>}<button type="button" data-default-focus="true" onClick={() => window.electronAPI?.openExternal?.(url)} className="flex-1 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-900/20 transition font-bold text-sm flex items-center justify-center gap-2">{t("ui.download.2d5146")}{' '}<ExternalLink size={14}/></button></div>
    </div>
  </div>;
};
