  ...
  return <>
    {context.tabs.map(tab => {
      ...
      return <div key={tab.id} data-native-tab={tab.id} data-titlebar-tab-id={tab.id} data-active-tab={tab.active} data-dragging={dragged === tab.id || undefined}
        ...
        title={!tab.ready ? `正在打开 ${tab.label}` : tab.pinned ? tab.label : `${tab.label} — 拖出到新窗口，拖回标签栏合并`}
        ...
        <button type="button" aria-busy={!tab.ready} onClick={() => { if (!suppressClick.current) run(api.activate(tab.id)); }} className="flex min-w-0 flex-1 items-center gap-2 self-stretch px-3 text-left">{tab.ready && tab.kind === 'component' ? <ComponentIcon src={tab.iconUrl} size={14}/> : <Icon size={14} className={tab.ready ? 'shrink-0' : 'shrink-0 animate-spin'}/>}<span className="truncate">{tab.label}</span></button>
        {!tab.pinned && <button type="button" data-tab-drag-ignore aria-label={`关闭 ${tab.label}`} onClick={() => run(api.close(tab.id))} className="mr-1 rounded p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-800"><X size={13}/></button>}
      </div>;
    })}
    {menu && <div role="menu" onPointerDown={event => event.stopPropagation()} className="app-titlebar-control fixed z-[4000] min-w-44 rounded-lg border border-slate-200 bg-white p-1 shadow-xl" style={{ left: Math.min(menu.x, window.innerWidth - 190), top: menu.y }}>
      <button role="menuitem" className="flex w-full items-center gap-2 rounded px-3 py-2 text-sm hover:bg-slate-100" onClick={() => { run(api.detach(menu.id)); setMenu(null); }}><ExternalLink size={14}/>移到新窗口</button>
      <button role="menuitem" className="w-full rounded px-3 py-2 text-left text-sm hover:bg-slate-100" onClick={() => { run(api.merge()); setMenu(null); }}>合并所有窗口</button>
    </div>}
  </>;
};
