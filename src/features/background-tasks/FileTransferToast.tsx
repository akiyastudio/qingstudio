...
export const FileTransferToastItem = ({ task, onMinimize, onDismiss, onPause, onContinue, onCancel }: { task: BackgroundTask } & FileTransferToastActions) => {
  ...
  const countUnit = metadata.filesCopied !== undefined || metadata.totalFiles !== undefined || ['copy', 'paste', 'import', 'import-project'].includes(operation) ? 'task.filesProgress' : 'task.itemsProgress';
  ...
  const details = [
    totalFiles > 0 ? t(countUnit, { done: filesCopied, count: totalFiles }) : '',
  ...
  return <div role={taskToastLiveRole(task.state)} data-top-toast-id={`task:${task.id}`} className={`file-transfer-toast ${failed ? 'border-red-200 bg-red-50' : completed ? 'border-emerald-200 bg-emerald-50' : ''}`}>
    <div className="flex min-w-0 items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-3 text-xs"><span className={`truncate font-bold ${failed ? 'text-red-800' : completed ? 'text-emerald-800' : 'text-blue-800'}`}>{renderText(message)}</span>{!queued && !failed && !completed && <span className="shrink-0 font-mono font-bold tabular-nums text-blue-700">{Math.round(progress)}%</span>}</div>
        ...
        <p className="mt-1 line-clamp-1 text-[11px] tabular-nums text-blue-600">{queued ? task.blockedByTaskIds?.length ? t("ui.starts.automatically.when.the.blocking.task.f653f1") : versionTracking ? t("ui.version.comparison.starts.automatically.when.complete.a334f0") : t("ui.starts.automatically.when.required.resources.are.34e316") : details || (versionTracking ? t("ui.confirm.version.matches.after.comparison.740de2") : renderText(task.title))}</p>
      </div>
      <div className="flex shrink-0 items-center gap-1">
        {failed || completed
          ? <button type="button" onClick={() => onDismiss(task.id)} aria-label={t("ui.dismiss.notification.27413c")} title={t("ui.dismiss.notification.27413c")} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"><X size={15}/></button>
          : <button type="button" onClick={() => onMinimize(task.id)} aria-label={t("ui.minimize.to.task.center.1f3d97")} title={t("ui.minimize.to.task.center.the.task.8c25e3")} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-blue-200 bg-white/70 text-blue-700 transition hover:border-blue-300 hover:bg-blue-100"><Minimize2 size={15}/></button>}
        {(task.state === 'running' || task.state === 'resuming') && task.capabilities.pausable && <button type="button" onClick={() => onPause(task.id)} aria-label={t("ui.pause.task.b8aa67")} title={t("ui.pause.task.b8aa67")} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-amber-700 transition hover:bg-amber-50"><Pause size={15}/></button>}
        {paused && task.capabilities.pausable && <button type="button" onClick={() => onContinue(task.id)} aria-label={t("ui.resume.task.f00c44")} title={t("ui.resume.task.f00c44")} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-blue-700 transition hover:bg-blue-50"><Play size={15}/></button>}
        {!failed && !completed && task.cancellable && <button type="button" onClick={() => onCancel(task)} aria-label={t("ui.cancel.task.537d17")} title={t("ui.cancel.task.537d17")} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-red-600 transition hover:bg-red-50"><X size={16}/></button>}
      </div>
    </div>
  </div>;
};
...
export const FileTransferToast = ({ stackRef, presentation, reflowKey }: { stackRef: React.RefObject<HTMLDivElement | null>; presentation: ReturnType<typeof useFileTransferToastPresentation>; reflowKey: string }) => {
  ...
  return <>
    {pendingAction && <span className="sr-only" role="status">{t("ui.applying.task.action.26c6c6")}</span>}
    ...
    {actionError && <div role="alert" className="file-transfer-toast border-red-200 bg-red-50 text-xs text-red-700"><LocalizedText value={actionError}/></div>}
    {overflowCount > 0 && <div data-top-toast-id="task-overflow" className="file-transfer-toast-overflow">{t("message.2f60df77b36a", { count: overflowCount })}</div>}
  </>;
};
