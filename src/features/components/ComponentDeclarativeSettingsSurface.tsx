...
export const ComponentDeclarativeSettingsSurface = ({ page }: { page: DeclarativePage }) => {
  ...
  const form = <div className="pf-settings-container"><header className="pf-settings-header"><h1 className="pf-settings-title">{renderText(page.pageTitle)}</h1></header>{saveError && <div role="alert" data-tone="danger" className="pf-callout"><LocalizedText value={saveError}/></div>}{Boolean(page.form.help?.length) && <section className="pf-settings-group" aria-label={t("ui.instructions.cb4ba4")}><h2 className="pf-settings-group-title">{t("ui.instructions.cb4ba4")}</h2><div className="pf-settings-card">{page.form.help?.map((item, index) => <div key={index} className="pf-settings-row"><div className="pf-settings-copy" style={{ gridColumn: '1 / -1' }}><h3 className="pf-settings-row-title">{item.title}</h3><p className="pf-settings-description whitespace-pre-line">{item.description}</p></div></div>)}</div></section>}{page.form.groups.map(group => <section key={group.id} className="pf-settings-group"><h2 className="pf-settings-group-title">{renderText(group.title)}</h2>{renderText(group.description) && <p className="pf-settings-group-description">{renderText(group.description)}</p>}<div className="pf-settings-card">{group.fields.map(field => <div key={field.id} className="pf-settings-row"><div className="pf-settings-copy"><label htmlFor={`component-setting-${field.id}`} className="pf-settings-row-title">{renderText(field.label)}</label>{renderText(field.description) && <p className="pf-settings-description">{renderText(field.description)}</p>}</div><div className="pf-settings-control"><FieldControl field={field} value={values[field.id] ?? field.default} saving={savingIds.has(field.id)} onDraft={value => draft(field.id, value)} onCommit={value => commit(field.id, value)}/></div></div>)}</div></section>)}{Boolean(page.form.notices?.length) && <section className="pf-settings-group"><h2 className="pf-settings-group-title">{t("ui.third.party.software.and.runtimes.4085ff")}</h2><div className="pf-settings-card">{page.form.notices?.map((notice, index) => <div key={index} className="pf-settings-row"><div className="pf-settings-copy"><h3 className="pf-settings-row-title">{renderText(notice.title)}</h3><p className="pf-settings-description">{renderText(notice.description)}</p></div><div className="pf-settings-control"><div className="pf-control-cluster"><span className="text-xs font-bold text-slate-500">{notice.license}</span><button type="button" className="pf-button inline-flex shrink-0 items-center justify-center gap-1.5 whitespace-nowrap" onClick={() => void window.electronAPI.openExternal(notice.sourceUrl)}>{t("ui.source.a488e9")}<ExternalLink size={13} className="shrink-0"/></button><button type="button" className="pf-button inline-flex shrink-0 items-center justify-center gap-1.5 whitespace-nowrap" onClick={() => void window.electronAPI.openExternal(notice.licenseUrl)}>{t("ui.license.310c62")}<ExternalLink size={13} className="shrink-0"/></button></div></div></div>)}</div></section>}</div>;
  if (page.renderMode === 'hybrid') {
    ...
    return <div className="pf-settings-page relative h-full overflow-hidden">
      ...
      {!pageReady && <div className="absolute inset-0 flex items-center justify-center p-6">
        {failure ? <section className="pf-card w-full max-w-xl p-6 text-center"><AlertTriangle size={24} className="mx-auto text-red-600"/><h2 className="mt-3 text-sm font-bold">{t("ui.could.not.load.component.settings.55c547")}</h2><p className="pf-settings-description mt-2">{failure}</p><button type="button" onClick={retryHybridLoad} className="pf-button mt-4 inline-flex items-center gap-2"><RotateCcw size={14}/>{t("ui.retry.b8784c")}</button></section> : <div className="flex items-center gap-2 text-sm font-bold"><Loader2 size={18} className="animate-spin text-blue-600"/>{t("message.e9e620f6bf4b", { value0: renderText(page.pageTitle) })}</div>}
      </div>}
    </div>;
  }
  if (loading) return <div className="pf-settings-page flex h-full items-center justify-center gap-2 text-sm font-bold"><Loader2 size={18} className="animate-spin text-blue-600"/>{t("message.5c227686f9a5", { value0: renderText(page.pageTitle) })}</div>;
  if (loadError) return <div className="pf-settings-page flex h-full items-center justify-center p-6"><section className="pf-card w-full max-w-xl p-6 text-center"><AlertTriangle size={24} className="mx-auto text-red-600"/><h2 className="mt-3 text-sm font-bold">{t("ui.could.not.load.component.settings.55c547")}</h2><p className="pf-settings-description mt-2"><LocalizedText value={loadError}/></p><button type="button" onClick={() => void load()} className="pf-button mt-4 inline-flex items-center gap-2"><RotateCcw size={14}/>{t("ui.retry.b8784c")}</button></section></div>;
