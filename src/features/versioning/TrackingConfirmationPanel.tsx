...
export const TrackingConfirmationPanel = ({ active, sessionId, workspacePath, progressFolders, cacheConfig, onClose, onCommitted, onReleased, onNotice }: TrackingConfirmationPanelProps) => {
  ...
  const sessionFailure = sessionActionsAvailable ? trackingSessionFailureNotice(view.session?.error) : undefined;
  ...
  return <div role="dialog" aria-modal="true" aria-label={t("ui.confirm.version.matches.fc68c2")} className="fixed inset-0 z-[350] flex items-center justify-center bg-slate-950/55 p-4">
    <div className="tracking-confirmation-dialog">
      <header className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h3 className="text-lg font-bold text-slate-800">{t("ui.confirm.version.matches.fc68c2")}</h3><p className="mt-1 text-xs text-slate-500">{t("message.9f096b885f42", { value0: parentFolder?.displayName || t("ui.previous.version.ffc5b2"), value1: progressFolder?.displayName || t("ui.current.version.837bc9"), value2: total, value3: unresolved })}</p></div><button type="button" disabled={committing || Boolean(busyItemId)} onClick={onClose} title={t("ui.continue.later.40c506")} className="rounded p-1.5 text-slate-500 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"><X size={17}/></button></header>
      {sessionFailure && <p className="border-b border-red-200 bg-red-50 px-5 py-2 text-xs text-red-700">{t(sessionFailure.key, { value0: sessionFailure.value0 })}</p>}
      {loading ? <div className="flex flex-1 items-center justify-center gap-2 text-sm text-slate-500"><Loader2 size={20} className="animate-spin"/>{t("ui.loading.match.results.f4f632")}</div> : loadError ? <div role="alert" className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center text-sm text-red-600"><p>{t("message.ab7d5d3d4f27", { value0: loadError })}</p><button type="button" onClick={() => void loadSession()} className="dialog-secondary">{t("ui.reload.7bdd5c")}</button></div> : <div className="tracking-confirmation-layout">
        <aside className="tracking-confirmation-list flex flex-col"><div className="min-h-0 flex-1 overflow-y-auto">{groups.map(group => <section key={group.category}>{group.items.length > 0 && <h4 className="sticky top-0 z-10 flex justify-between border-y border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold text-slate-500"><span>{categoryLabel[group.category]}</span><span>{group.items.length}</span></h4>}{group.items.map(item => <button key={item.id} data-tracking-item-id={item.id} type="button" aria-pressed={item.id === view.selectedItemId} onClick={() => setView(current => ({ ...current, selectedItemId: item.id }))} className={`tracking-confirmation-row ${item.id === view.selectedItemId ? 'is-selected' : ''}`}><span className="min-w-0"><b className="block truncate" title={item.sourceName || item.targetName}>{item.sourceName || item.targetName || t("ui.unnamed.media.fa9dc7")}</b><small className="mt-1 block truncate text-slate-400">{item.referenceName ? t("ui.previous.value0.40bb66", { value0: item.referenceName }) : t("ui.no.previous.version.link.566fae")}</small></span><span className={`tracking-confirmation-status is-${item.status}`}>{statusLabel[item.status]}</span><small className="col-span-2 text-right text-slate-400">{item.confidence || '—'}</small></button>)}</section>)}</div><div className="flex items-center justify-between border-t border-slate-200 bg-white px-3 py-2 text-[11px] text-slate-500"><button type="button" disabled={pageCursor === 0 || Boolean(busyItemId) || committing} onClick={() => setPageCursor(Math.max(0, pageCursor - TRACKING_PAGE_SIZE))} className="dialog-secondary !px-2 !py-1">{t("ui.previous.page.c9b9ae")}</button><span>{total ? `${pageCursor + 1}–${pageEnd} / ${total}` : t("ui.0.items.c35a86")}</span><button type="button" disabled={view.nextCursor === undefined || Boolean(busyItemId) || committing} onClick={() => view.nextCursor !== undefined && setPageCursor(view.nextCursor)} className="dialog-secondary !px-2 !py-1">{t("ui.next.page.8a8542")}</button></div></aside>
        <main className="tracking-confirmation-main">{selectedItem ? <><div className="mb-2 flex items-center justify-between gap-2 text-xs text-slate-500"><span className="truncate"><b className="text-slate-700">{selectedItem.sourceName || selectedItem.targetName}</b> · {statusLabel[selectedItem.status]} · {selectedItem.confidence || t("ui.no.confidence.score.41ee06")}</span><div className="flex shrink-0 gap-1"><button type="button" onClick={() => setView(current => ({ ...current, selectedItemId: adjacentTrackingPreviewItemId(current.items, current.selectedItemId, -1) }))} className="dialog-secondary !px-2 !py-1" title={t("ui.previous.item.b88f02")}><ArrowLeft size={14}/></button><button type="button" onClick={() => setView(current => ({ ...current, selectedItemId: adjacentTrackingPreviewItemId(current.items, current.selectedItemId, 1) }))} className="dialog-secondary !px-2 !py-1" title={t("ui.next.item.f8baad")}><ArrowRight size={14}/></button></div></div><ProgressPairPreview referencePath={paths.referencePath} sourcePath={paths.sourcePath} referenceLabel={selectedItem.referenceName ? t("ui.previous.version.value0.1f74fd", { value0: selectedItem.referenceName }) : t("ui.previous.version.ffc5b2")} sourceLabel={t("ui.current.version.value0.319cee", { value0: selectedItem.sourceName || selectedItem.targetName || '' })} referenceMissing={paths.referenceMissing} mode={mode} swapped={swapped} cacheConfig={cacheConfig} onModeChange={setMode} onSwappedChange={setSwapped}/><div className="tracking-confirmation-decisions">{unresolvedTrackingStatus(selectedItem.status) && !missingCurrentItem && <div className="flex min-w-[240px] flex-1 gap-2"><input value={relocateName} onChange={event => setRelocateName(event.target.value)} placeholder={t("ui.enter.a.file.name.from.the.93d508")} className="min-w-0 flex-1 rounded-md border border-slate-300 px-3 py-2 text-xs"/><button type="button" disabled={!sessionActionsAvailable || !relocateName.trim() || Boolean(busyItemId)} onClick={() => void decide('accepted', relocateName.trim())} className="dialog-secondary">{t("ui.match.previous.version.cbaf4f")}</button></div>}{unresolvedTrackingStatus(selectedItem.status) && <><button type="button" disabled={!sessionActionsAvailable || Boolean(busyItemId)} onClick={() => void decide('rejected')} className="dialog-secondary">{missingCurrentItem ? t("ui.confirm.missing.from.current.version.ec1c9a") : hasReferenceCandidate ? t("ui.not.the.same.image.eeeb5e") : t("ui.mark.as.unrelated.1230e5")}</button>{!missingCurrentItem && hasReferenceCandidate && selectedItem.status === 'pending_confirmation' && <button type="button" disabled={!sessionActionsAvailable || Boolean(busyItemId)} onClick={() => void decide('accepted')} className="dialog-primary">{t("ui.confirm.same.image.e1bc8e")}</button>}</>}</div></> : <div className="flex flex-1 items-center justify-center text-sm text-slate-400">{t("ui.no.images.need.confirmation.d0dcdd")}</div>}</main>
      </div>}
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4"><button type="button" disabled={!sessionActionsAvailable || committing || Boolean(busyItemId)} onClick={() => void release()} className="text-xs font-medium text-red-600 hover:text-red-700 disabled:opacity-40">{t("ui.discard.this.confirmation.28e64d")}</button><div className="flex gap-2"><button type="button" disabled={committing || Boolean(busyItemId)} onClick={onClose} className="dialog-secondary">{t("ui.continue.later.40c506")}</button>{commitUnavailable ? <button type="button" disabled={!sessionActionsAvailable || committing || loading || Boolean(loadError) || unresolved === 0} onClick={revealPending} className="dialog-primary">{loadError ? t("ui.cannot.submit.9767c2") : t("ui.images.need.confirmation.15bbf9")}</button> : <button type="button" disabled={!sessionActionsAvailable || committing} onClick={() => void commit()} className="dialog-primary inline-flex items-center gap-2">{committing && <Loader2 size={15} className="animate-spin"/>}{t("ui.submit.results.a3a691")}</button>}</div></footer>
    </div>
  </div>;
};
