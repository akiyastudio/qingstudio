...
import { PROJECT_FILE_FILTER_OPTIONS, PROJECT_BINARY_RATING_FILTER_OPTIONS, PROJECT_STAR_RATING_FILTER_OPTIONS, type ProjectFileFilter, type ProjectRatingFilter } from './project-file-filter-options';
...
import { usePreviewDecoders, decoderForFile } from './ComponentDecodedPreview';
...
const FileBrowserWorkspace = ({ pageId, active, activeView, project, workspacePath, componentWorkspacePath = workspacePath, inspirationTargetWorkspacePath, inspirationLibraryRootPath, installedComponentIds: _installedComponentIds, videoToolsAvailable, advancedVideoPlaybackAvailable, componentHostActions = [], componentContributions = [], onOpenComponentPage = () => undefined, videoPlaybackSettings, projectToolbar = { order: [...PROJECT_TOOLBAR_ACTION_IDS], hidden: [], onlyShowAvailable: false }, customProjectCategories = [], projectCategoryOrder = [], progressNamePresets = [], initialPanel, initialRelativePath = '', importConfig, importDefaults, brollConfig, videoTools, matchConfig, researchConfig, mediaCacheConfig, defaultFolderSort, itemOpenMode, folderAlphabetFilterEnabled = true, versionTreeEnabled = true, favoriteDisplayMode = 'binary', browserContext, navigationRequest, onDirectoryChange, onOpenInspirationPath, onOpenDirectoryPage, onOpenToolTab = () => undefined, onCloseToolTab = () => undefined, onImportConfigChange, onMatchConfigChange, onResearchConfigChange, onNotice, onProjectMoved = () => undefined, onDeleted = () => undefined }: FileBrowserWorkspaceProps) => {
  ...
  const handleProjectImportRecovery = async (result: Awaited<ReturnType<typeof projectWorkspaceClient.importProjectFiles>>) => {
    ...
    await appDialog.alert({ title: '导入结果已保留，需要恢复处理', message: `软件已刷新目录和版本树。保留的文件仍然有效，请勿直接重复导入同一来源。${cleanupSummary}`, detail: result.error, confirmLabel: '我知道了' });
  ...
  const renderPlacedToolActions = (placement: WorkspaceToolPlacement, contextMenu = false) => menuContributions(placement).map(contribution => {
    const available = contextMenu ? fileMenuHasSafeToolSelection : toolbarHasSafeToolSelection;
    return <button key={`${contribution.componentId}:${contribution.contributionId}`} type="button" disabled={!available} title={available ? contribution.title : '请选择文件或文件夹'} className="project-menu-item" onClick={event => {
      event.stopPropagation();
  ...
  const projectToolbarButtons: Record<ProjectToolbarActionId, React.ReactNode> = {
    ...
    'office-extract': <div className="project-toolbar-tool-group relative" onClick={event => event.stopPropagation()}>
      <button type="button" onClick={() => { const next = !showOfficeToolsMenu; window.dispatchEvent(new Event('photoflow-menu-open')); setShowOfficeToolsMenu(next); }} title="Office 文档" aria-label="Office 文档" aria-haspopup="menu" aria-expanded={showOfficeToolsMenu} className={`project-action-button ${showOfficeToolsMenu || panel === 'office-extract' ? 'bg-blue-50 text-blue-600' : ''}`}><FileImage size={16}/>Office 文档<ChevronDown size={13}/></button>
      {showOfficeToolsMenu && <div className="project-toolbar-tool-submenu absolute left-0 top-full z-50 mt-1 w-52 rounded-lg border border-slate-200 bg-white p-1 shadow-xl">
        <button type="button" disabled={!selectedOfficeExtractEntries.length} onClick={() => { setShowOfficeToolsMenu(false); setShowToolbarOverflowMenu(false); openOfficeImageExtractor(selectedOfficeExtractEntries); }} title={selectedOfficeExtractEntries.length ? `从所选 ${selectedOfficeExtractEntries.length} 个 Office 文档提取图片` : '请选择 Office 文档'} className="project-menu-item"><FileImage size={14}/>提取文档图片</button>
  ...
  return (
    <div ref={projectWorkspaceRef} className="flex h-full w-full min-w-0 flex-col animate-in fade-in duration-300">
      ...
      {fileMenu && createPortal(<ViewportContextMenu x={fileMenu.x} y={fileMenu.y} widthClass="w-52" allowSubmenus>
        ...
        {(fileMenuHasVideoTarget || fileMenuHasPlacedToolAction('workspace.videoTools')) && <ViewportSubmenu><button type="button" aria-haspopup="menu" aria-expanded={false} className="project-menu-item w-full"><Video size={14}/>视频工具<span className="ml-auto">›</span></button><div className="z-[302] w-52 rounded-lg border border-slate-200 bg-white p-1 shadow-xl transition">{fileMenuHasVideoTarget && <button className="project-menu-item" onClick={() => { const entries = fileMenuEntries; setFileMenu(null); void openResearchForEntries(entries); }}><Video size={14}/>截取分镜帧</button>}{fileMenuHasVideoTarget && videoTranscodeContribution && <button className="project-menu-item" onClick={() => { const targets = fileMenuTargetPaths; setFileMenu(null); openVideoToolContribution(videoTranscodeContribution, targets); }}><Gauge size={14}/>视频转码</button>}{fileMenuHasVideoTarget && videoSplitContribution && <button disabled={!fileMenuHasVideoSplitTarget} title={fileMenuHasVideoSplitTarget ? `将所选 ${fileMenuEntries.length} 个视频或文件夹中的视频无损切成约 3.95 GB 的连续分段` : '请选择视频或文件夹'} className="project-menu-item" onClick={() => { const targets = fileMenuEntries.map(entry => entry.relativePath); setFileMenu(null); if (!targets.length) return; openVideoToolContribution(videoSplitContribution, targets); }}><Cut size={14}/>视频切割</button>}{renderPlacedToolActions('workspace.videoTools', true)}</div></ViewportSubmenu>}
        ...
        {(officeImageExtractorAvailable && fileMenuOfficeEntries.length > 0 || fileMenuHasPlacedToolAction('workspace.officeTools')) && <ViewportSubmenu>
          <button type="button" aria-haspopup="menu" aria-expanded={false} className="project-menu-item w-full"><FileImage size={14}/>Office 文档<span className="ml-auto">›</span></button>
          <div className="z-[302] w-52 rounded-lg border border-slate-200 bg-white p-1 shadow-xl transition">
            <button disabled={!fileMenuOfficeEntries.length} className="project-menu-item" onClick={() => { const entries = fileMenuOfficeEntries; setFileMenu(null); openOfficeImageExtractor(entries); }}><FileImage size={14}/>提取文档图片{fileMenuOfficeEntries.length > 1 ? `（${fileMenuOfficeEntries.length} 个文档）` : ''}</button>
      ...
      <div data-project-overview="true" className="flex flex-wrap items-start justify-between gap-3 pt-6">
        ...
        <WorkspacePanelDragSpace id="files" label="文件夹"/>
      ...
      {folderAlphabetFilterVisible && <nav aria-label="按文件夹首字母筛选" className="-mt-3 flex flex-wrap items-center gap-1 px-6 pb-2 pt-1">
