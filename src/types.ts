...
export interface IElectronAPI {
  ...
  getComponentThumbnail: (request: Omit<import('./contracts/component-preview').PreviewDecodeRequest, 'pageIndex'>) => Promise<import('./contracts/component-preview').PreviewDecodeResult>;
  cancelComponentPreview: (requestId: string) => Promise<{ success: boolean }>;
  componentPreviewPlayback: (request: import('./contracts/component-preview').PreviewPlaybackRequest) => Promise<import('./contracts/component-preview').PreviewPlaybackResult>;
  onComponentPreviewReleased: (callback: (event: { previewId: string }) => void) => () => void;
  ...
  controlVideoPlayer: (sessionId: string, request: { action: 'play' | 'pause' | 'seek' | 'frame-step' | 'frame-back-step' | 'volume' | 'mute' | 'speed' | 'stop' | 'subtitle-select' | 'subtitle-visible' | 'subtitle-delay' | 'subtitle-style' | 'audio-select' | 'transform' | 'hdr-mode' | 'tone-mapping' | 'statistics-level' | 'fullscreen'; value?: number | boolean | string; fontSize?: VideoPlaybackSettings['subtitleSize']; style?: VideoPlaybackSettings['subtitleStyle']; transform?: { aspectMode: 'source' | 'contain' | 'cover' | '16:9' | '4:3' | '1:1'; rotation: 0 | 90 | 180 | 270; flipHorizontal: boolean; flipVertical: boolean; crop?: {x:number;y:number;width:number;height:number} }; hdrMode?: VideoPlaybackSettings['hdrMode']; toneMapping?:VideoPlaybackSettings['toneMapping'];targetPeakNits?:number; statisticsLevel?: 'off' | 'basic' | 'detailed' }) => void;
